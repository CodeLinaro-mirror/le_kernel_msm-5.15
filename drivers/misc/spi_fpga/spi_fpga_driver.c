...
/*
 *  spi_fpga_drivers.c - QED spi fpga driver.
 *
...
static int fpga_spi_probe(struct platform_device *pdev)
{
        ...
        if (rc) {
                dev_dbg(dev, "Failed to read config bus number, using default: %d\n", rc);
                state->cfg_info.bus_num = FPGA_CFG_SPI_DEV_CFG;
        } else {
                state->cfg_info.bus_num = spi_bus;
                dev_dbg(dev, "Config bus number from of: %d\n", spi_bus);
        }
